...
Scans new records, captures field names and their distinct values,
...
def build_schema_snapshot(records_queryset) -> Dict[str, Dict[str, Any]]:
    ...
    Args:
        records_queryset: QuerySet of Record objects to analyze
...
def aggregate_all_entities(chunk_size: int = 1000) -> Dict[str, Any]:
    ...
    try:
        # Get all tenants that have records
        tenants_with_records = Tenant.objects.filter(
            id__in=Record.objects.values('tenant_id').distinct()
        )
