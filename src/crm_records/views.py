...
class GetNextLeadView(APIView):
    """
    Get and assign the next available lead using the bucketed ``LeadPipeline``.

    All RMs (including SELF TRIAL) use tenant ``Bucket`` + ``UserBucketAssignment`` ordering,
    ``BucketQuerysetBuilder`` filters, ``CallAttemptMatrix`` exclusions when applicable,
    daily-limit gating on fresh buckets, and assignment via ``LeadPipeline``.
    """
    permission_classes = [IsTenantAuthenticated]

    @extend_schema(
        summary="Get next lead from queue",
        description="Atomically fetches and assigns the next available lead using the lead pipeline "
        "(bucket priority, routing rules, call-attempt matrices, daily limit). "
        "Lead filters (party, lead source, lead status, routing) are loaded from the database only.",
        responses={
            200: OpenApiResponse(
    ...
    def get(self, request):
        ...
        logger.info(
            ...
            filters.eligible_lead_types,
            filters.eligible_lead_sources or "(none)",
            filters.eligible_lead_statuses or "(none)",
            filters.daily_limit,
        )

        logger.info(
            "[GetNextLead] Step 2b: bucketed LeadPipeline user=%s tenant=%s debug=%s — "
            "see [LeadPipeline] logs for buckets, daily_limit, matrix, assignment",
            user_identifier,
            tenant.id if tenant else None,
            debug_mode,
        )
