...
class TrialActivationSerializer(serializers.Serializer):
    """
    Lightweight serializer to validate trial activation payloads.
    """
...
class TrialActivationView(APIView):
    """
    Record a trial activation event for the current user and tenant.
    """
    permission_classes = [IsTenantAuthenticated]
    ...
    def post(self, request):
        ...
        record = Record.objects.create(
            ...
            data={
                "user_supabase_uid": supabase_uid,
...
class TrialActivationStatsView(APIView):
    """
    Return today's trial activation count for the current user.
    """
    permission_classes = [IsTenantAuthenticated]
    ...
    def get(self, request):
        ...
        # Build start/end of today in the current timezone
        ...
        qs = Record.objects.filter(
            ...
            data__user_supabase_uid=supabase_uid,
        )

        return Response(
            {
                "count": qs.count(),
                "daily_target": None,  # No target configured yet
            },
            status=status.HTTP_200_OK,
        )
