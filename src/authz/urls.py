from django.urls import path
from .views_management import (
    ListTenantUsersView,
    RolesView,
    CurrentUserRoleView,
    UpdateUserHierarchyView,
)

urlpatterns = [
    path("users/", ListTenantUsersView.as_view(), name="authz_list_users"),
    path("users", ListTenantUsersView.as_view(), name="authz_list_users_no_slash"),  # Support both
    path("users/hierarchy/", UpdateUserHierarchyView.as_view(), name="authz_update_user_hierarchy"),
    path("roles/", RolesView.as_view(), name="authz_roles"),  # With trailing slash for frontend compatibility
    path("roles", RolesView.as_view(), name="authz_roles_no_slash"),  # Support both
    path("me/role/", CurrentUserRoleView.as_view(), name="authz_current_user_role"),
]
