...
class TestPyroJobCreatorScheduling:
    ...
    def _run_pyro_job_creator_tick(self, schedule):
    ...
    def test_first_run_schedules_immediately(self):
        ...
        from pyro_jobs.models import PyroJob
        self._run_pyro_job_creator_tick({"test_job": {"every_minutes": 60}})
    ...
    def test_schedules_multiple_jobs_independently(self):
        ...
        self._run_pyro_job_creator_tick(schedule)
...
# ══════════════════════════════════════════════════════════════════════
# PYRO JOB EXECUTOR TESTS
# ══════════════════════════════════════════════════════════════════════

@pytest.mark.django_db
class TestPyroJobExecutorExecution:

    def _run_pyro_job_executor_tick(self, handlers):
    ...
    def test_successful_job_marked_completed(self):
        from pyro_jobs.models import PyroJob
        job = make_job()
        self._run_pyro_job_executor_tick({"test_job": lambda p: None})
        job.refresh_from_db()
    ...
    def test_successful_job_handler_receives_payload(self):
        ...
        self._run_pyro_job_executor_tick({"test_job": lambda p: received.append(p)})
    ...
    def test_no_handler_marks_failed(self):
        ...
        self._run_pyro_job_executor_tick({})
        job.refresh_from_db()
    ...
    def test_first_failure_schedules_retry(self):
        ...
        self._run_pyro_job_executor_tick({"test_job": crash})
        job.refresh_from_db()
        assert job.status == PyroJob.STATUS_PENDING
        assert job.run_at > timezone.now()
    ...
    def test_job_marked_running_before_execution(self):
        ...
        self._run_pyro_job_executor_tick({"test_job": check_status})
    ...
    def test_attempts_incremented_on_each_run(self):
        ...
        for _ in range(3):
            ...
            if job.status == PyroJob.STATUS_PENDING and job.run_at <= timezone.now():
                self._run_pyro_job_executor_tick({"test_job": sometimes_fail})
    ...
    def test_multiple_jobs_all_executed(self):
        ...
        executed = []
        self._run_pyro_job_executor_tick({"test_job": lambda p: executed.append(p["i"])})
...
class TestEdgeCases:
    ...
    def test_empty_queue_pyro_job_executor_returns_none(self):
        from pyro_jobs.models import PyroJob
        from pyro_jobs.pyro_job_executor import fetch_and_lock_job
        job = fetch_and_lock_job(PyroJob)
        assert job is None
    ...
    def test_pyro_job_executor_started_at_set_correctly(self):
    ...
    def test_pyro_job_creator_schedule_once_with_past_run_at(self):
...
class TestConcurrency:
    ...
    def _pyro_job_creator_tick(self, schedule):
    ...
    def test_concurrent_pyro_job_creator_workers_dont_duplicate(self):
        """
        ...
        def pyro_job_creator_tick():
            try:
                self._pyro_job_creator_tick({"test_job": {"every_minutes": 60}})
            except Exception as e:
                errors.append(e)

        threads = [threading.Thread(target=pyro_job_creator_tick) for _ in range(3)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
    ...
    def test_concurrent_pyro_job_executor_workers_each_run_different_job(self):
