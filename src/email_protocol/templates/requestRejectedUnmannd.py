...
def build_request_rejected_unmannd_email(context: Dict[str, Any]) -> Tuple[str, str, str]:
    """
    Returns (subject, text_body, html_body) when a request is rejected.
    """
    ...
    status_text = context.get("status_text", "REJECTED")
    ...
    subject = f"[Pyro] Your request #{request_id} was rejected"

    text_body = (
        f"Hi {requester_name},\n\n"
        f"Your request in {tenant_name} has been rejected.\n\n"
      ...
      <tr>
        <td align="center">
          <table role="presentation" width="620" cellspacing="0" cellpadding="0" style="max-width:620px;background:#ffffff;border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;">
            <tr>
              <td style="padding:18px 22px;background:linear-gradient(135deg,#9f1239 0%,#e11d48 100%);">
                <div style="font-size:13px;letter-spacing:.08em;text-transform:uppercase;color:#fecdd3;">Pyro Notifications</div>
            ...
            <tr>
              <td style="padding:22px;">
                <p style="margin:0 0 14px;font-size:15px;color:#374151;">
                  Hi {requester_name}, your request has been rejected.
                </p>
                ...
                <div style="margin-top:20px;">
                  <a
                    ...
                    style="display:inline-block;background:#e11d48;color:#ffffff;text-decoration:none;font-size:14px;font-weight:600;padding:11px 16px;border-radius:10px;"
                  >
                    Open Request
                  </a>
                </div>
