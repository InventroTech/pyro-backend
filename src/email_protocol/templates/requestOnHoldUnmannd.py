...
def build_request_on_hold_unmannd_email(context: Dict[str, Any]) -> Tuple[str, str, str]:
    """
    Returns (subject, text_body, html_body) when a request is put on hold.
    """
    ...
    text_body = (
        f"Hi {requester_name},\n\n"
        f"Your request in {tenant_name} has been put on hold.\n\n"
      ...
      <tr>
        <td align="center">
          <table role="presentation" width="620" cellspacing="0" cellpadding="0" style="max-width:620px;background:#ffffff;border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;">
            <tr>
              <td style="padding:18px 22px;background:linear-gradient(135deg,#92400e 0%,#d97706 100%);">
                <div style="font-size:13px;letter-spacing:.08em;text-transform:uppercase;color:#fde68a;">Pyro Notifications</div>
            ...
            <tr>
              <td style="padding:22px;">
                <p style="margin:0 0 14px;font-size:15px;color:#374151;">
                  Hi {requester_name}, your request has been put on hold.
                </p>
